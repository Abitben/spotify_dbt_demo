SELECT 
    ...
    {{ dbt_utils.generate_surrogate_key(['albums.album_id','albums.album_name','albums.artist_id']) }} as album_key,
    {{ dbt_utils.generate_surrogate_key(['artists.artist_id']) }} as artist_key,
    {{ dbt_utils.generate_surrogate_key(['tracks.track_id', 'tracks.track_number', 'tracks.track_popularity']) }} as track_key, 
